...
BSONCXX_INLINE char* make_copy_for_bson(stdx::string_view s, uint32_t* len_out = nullptr) {
    if (s.empty()) {
        return nullptr;
    }
    ...
    // Determine whether the string_view includes the null byte.
    auto c = s.back();
    if (c != '\0') {
        length += 1;
    }

    char* copy = (char*)bson_malloc0(length);
    std::memset(copy, '\0', length);
    std::memcpy(copy, s.data(), s.length());

    if (len_out) {
        *len_out = (uint32_t)length;
    }
